...
def prepareDF(typ):
    rdds = [sc.wholeTextFiles("E:\\homework\\big data\\hw3\\20news-bydate-" + typ + "/" + category) \
                .map(lambda x: LabeledDocument(categoryFromPath(x[0]), x[1])) \
            for category in categories]
    return sc.union(rdds).toDF()
...
test_df = test_df.rdd.zipWithIndex().map(lambda x: (x[0][0], x[0][1], x[1])).toDF()
test_df.createOrReplaceTempView("test")
test_df = sql.sql("select _1 as category, _2 as text, _3 as id from test")
test_df.show()

test_df.rdd.collect()
# test_df.rdd.toDF().select(format_string('%s\t%s, %s, %s, %s',
#                                 test_df.category,
#                                 test_df.text,
#                                 test_df.id)) \
#     .write.save("report1", format="text")
...
# """
# schema = []
# for i in range(0, len(pred.schema.fields)):
...
# for id in ids:
#     #report details
#     doc = test_df.where("id == %s" % id)
#     true_category = doc.select("category").collect()[0][0]
#     text = doc.select("text").collect()[0][0]
#     prob = classifier_fn(text)[0][0]
#     predicted_category = class_names[0] if prob > 0.5 else class_names[1]
#     explanations = explainer.explain_instance(text, classifier_fn, num_features=6)
#     temp = [true_category, prob, predicted_category, explanations.as_list(())]
#     results.append(temp)
#
...
misclassified_docs = test_df.rdd.filter(lambda x: filter_doc(x)).collect()#.map(lambda x: map_doc(x)).collect()

########################################################################################################
""" Task 1.5
Get the word and summation weight and frequency
"""
# the way to get all words in a document
tokenizer.transform(misclassified_docs).select("words").show()
